...
- hook: the primary opening technique used in the first 3-5 seconds — Question (direct question to the viewer), Bold statement (strong claim or opinion), Pattern interrupt (surprising, unusual, or attention-grabbing opening), Before/after (immediately shows a transformation or final result), Text-on-screen (the opening hook is primarily written on screen), Problem/pain point (opens with a viewer problem or frustration), Curiosity/teaser (creates curiosity without revealing the answer), Demonstration/result (opens by showing the product, action, recipe, or result), or None (no clear hook is present). Base the value only on the opening moment; do not infer a hook if none is visible or audible there.
...
- cta: the primary call-to-action the creator makes — Follow, Buy now, Link in bio, Comment, Save, Share, Use promo code, Visit website, DM/Contact, Enter giveaway, or None (no explicit CTA is present). A CTA must be explicit — spoken or shown on screen. Do not treat a product mention, creator watermark, brand logo, or generic sign-off as a CTA. If several CTAs appear, choose the single most commercially relevant one (the one a brand would care about) as the value, rather than trying to represent all of them.
...
Every vocabulary also accepts "unknown". Use it whenever the clip does not give you enough to decide, and pair it with a low confidence and an empty evidence list. Recording "unknown" is correct behaviour; guessing a plausible-looking label is not. Note that values like "None", "Not present", "No brand or product present", and "No disclosure detected" differ from "unknown": the former mean you positively observed the absence of the thing, "unknown" means you could not tell either way. Use an empty evidence list for "None" and "unknown".

Two fields describe the clip in free text instead of a closed vocabulary, but are still scored objects (value/confidence/evidence). Each `value` must be plain, factual, at most 15 words, and contain no interpretation:
- visualDescription: what is visibly happening — framing (close-up, wide, handheld), whether/how the creator is shown, editing (cuts, transitions, overlays), lighting, and any on-screen text. Describe only what is seen. Do not repeat the visualStyle label (e.g. "Clean") or state a style judgment, and do not infer intent or mood.
- audioDescription: what is audibly happening — spoken language, voice delivery (pace, volume), narration vs. dialogue, background music, and sound effects. Describe only what is heard. Do not infer the creator's personality, emotion, or intent (that belongs to tone/sentiment, not here).

Two further fields are not scored against a vocabulary:
